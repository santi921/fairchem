...
@registry.register_model("general_esen_backbone_dens")
class General_eSEN_DeNS_Backbone_LR(nn.Module, GraphModelMixin):
    def __init__(
        self,
        max_num_elements: int = 100,
        sphere_channels: int = 128,
        ...
        self.irreps_sh = o3.Irreps.spherical_harmonics(lmax=self.lmax, p=1)
        self.force_embedding = SO3_Linear(
            in_features=1, out_features=self.sphere_channels_sum, lmax=self.lmax
        )
...
@registry.register_model("esen_linear_force_head_dens_lr")
class Linear_Force_Head_DeNS_LR(nn.Module, HeadInterface):
    def __init__(self, backbone):
        super().__init__()
        self.linear = SO3_Linear(backbone.sphere_channels_sum, 1, lmax=1)
        self.denoising_linear = SO3_Linear(backbone.sphere_channels_sum, 1, lmax=1)
