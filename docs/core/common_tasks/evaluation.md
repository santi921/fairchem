...
:::{tip}
Results will be logged according to the specified logger. We currently only support Weights and Biases.
:::
...
- `eval_unit`: The specification of the `MLIPEvalUnit` to be used
  ...
  - `model`: Defines how to load a pretrained model. We recommend using the `fairchem.core.units.mlip_unit.mlip_unit.load_inference_model` function
...
:::{admonition} Example Config Groups
:class: dropdown
